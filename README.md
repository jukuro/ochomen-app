...
- **きょう / 思い出 / 予定 / 買い物 / わが家** の 5 タブ UI（Phase 3）
- 思い出タブ：書類・お帳面・日記を統合
- 日記・音声入力
...
| 症状 | 原因 | 対処 |
|---|---|---|
| スキャンが「APIキー未設定」エラー | `GEMINI_API_KEY` が未設定 | Vercel の Environment Variables に `GEMINI_API_KEY` を追加し再デプロイ。ローカルは `.env.local` に設定 |
